...
| **New vehicles and parts**: a Paper Boat that drives off the quay and floats on the sea (in all three free-roam areas, and it leaps back up onto the quay), a Lantern Balloon that hovers and drifts down slowly after a hop, a Town Bicycle and a Tuk-Tuk Racer. Garage parts for every vehicle: roof rack, kayak and a string of lanterns; gloss, matte, glitter and chrome paint; classic, spoked, whitewall and slick wheels; twin, side and stack exhausts; six engine sounds and six horns (the horn plays when you pick it) | ✅ | `src/models/Vehicles.ts`, `src/audio/VehicleSounds.ts`, `src/gameplay/FreeRoam.ts` |
| **Characters**: an emote wheel (G, or 🙏 on touch) with wave, Ayubowan, cheer, dance, clap, bow, laugh and sit down, seen by other players too; pets that follow you on foot (a paper cat, a little fox, and an origami crane found in loot chests); up to 8 saved outfits; a Sri Lankan outfit pack (osariya, national dress tunic and cap, sari, sarong) with two ready-made outfits; group photos: Photo mode → *Group photo* invites nearby players, who stand beside you with one click, then a 3-2-1 countdown takes the picture | ✅ | `src/models/Human.ts`, `src/models/Pets.ts`, `src/ui/EmoteWheel.ts`, `src/gameplay/Profile.ts` |
| **Accounts (optional)**: Menu → Account to sign in with a name and password (no email). Progress is kept in the cloud and synced every 20 s or on demand; when two devices both changed, you choose which to keep. Friends (request, accept, see who's online and join their room) and clubs (create with a tag, join, club room, owner can remove members). Signed-in names show a ✓ in multiplayer, and a guest using a registered name is shown as "name guest". Passwords are salted scrypt hashes; sign-in tokens are stored hashed; logins lock after 10 wrong tries; 5 new accounts per address per hour. Friends and clubs follow the family settings | ✅ | `server/accounts.mjs`, `src/net/Account.ts`, `src/ui/AccountScreen.ts` |
| **Creativity**: a **road gallery** (Menu → Road gallery): publish roads from the Road Studio, play other players' roads, rate them 1–5 stars (not your own), sort by top/new/this week, search; three reports hide a road until an admin keeps or removes it. A **weekly contest** with a rotating theme: the three best-rated roads of the week win ink. **Mural walls**: two paintable boards in each free-roam area (press E, paint with the livery painter; saved with your progress). **Sticker book**: pages for each area, chapters, the photo hunt and the garage fill in from what you've done; a full page pays 150 ink once | ✅ | `server/gallery.mjs`, `src/ui/GalleryScreen.ts`, `src/world/Murals.ts`, `src/gameplay/Stickers.ts` |
| **Playing together** (Menu → Multiplayer → Play together, in any free-roam area): **convoys** (lead one and everyone in the room is invited; followers who keep within 40 m while moving earn ink, the leader earns per follower), 90-second **drift and stunt contests** with a live leaderboard (scores from others are sanity-capped), and a 3-minute **co-op paint splash**: 40 paint pots appear in the same places for everyone and the team collects them towards a shared target. Contests and paint splashes also work alone; invites from blocked players are ignored. Friends and clubs are in Menu → Account | ✅ | `src/gameplay/Together.ts`, `src/core/Game.ts` |
| **Chapter 5 · Postcards**: the Nile at Giza (date palms, feluccas, the three pyramids and the Sphinx), Santorini (white houses on terraces, blue domes, windmills), Kyoto by night (machiya lanes, lanterns, the Yasaka pagoda, fireflies — always night), Kandy in the rain (the lake, the cloud wall, the Temple of the Tooth) and Ella in the rain (tea hills, the little blue train, the Ella Gap, a waterfall). Districts can now set their own time and weather. Six missions; its gate stands in Serendib City | ✅ | `src/world/chapters/postcards.ts`, `src/world/dress/postcards.ts`, `src/models/LandmarksPostcards.ts` |
| **Festivals and seasons**: Vesak (star lanterns and a lit thorana), Sinhala & Tamil New Year (swing, oil lamps, the pot game, flowering trees) and Diwali (diyas, rangoli, lamp strings) dress the free-roam towns on their days (Vesak and Diwali follow the moon, calculated in the game) or whenever chosen in Settings → Accessibility. Seasons tint leaves and grass (blossom, autumn orange, winter frost) by your calendar, with the southern hemisphere and the tropics handled | ✅ | `src/world/Calendar.ts`, `src/world/FestivalDecor.ts` |
| **More to find and wear**: **20 hidden pockets** (5 in Harbour Town, 5 in Lantern Village, 10 in Serendib City): little scenes off the roads and off the map (a picnic, harbour cats, a lost easel, a stone circle, a message in a bottle, a moss shrine, a hammock…). Each pays 40 ink once and then shows on the map. **100 trophies** (from 30), with progress bars for pockets, all chapters, emotes, murals, stickers, the gallery, contests, convoys, pets, festivals and more. **Radio: 8 stations, 64 tracks**: Paper Kite FM, Harbour Hum, Midnight Ink, Serendib Beat, and new Baila Nights, 8-Bit Brush, Blue Hour Jazz and Monsoon Raga; each track has its own chord progression and arpeggio. **Wardrobe: 131 items** (from 70): 5 hairstyles, 9 hats, 5 glasses, a jacket, waistcoat and kurta, cargo trousers and a long skirt, new eyes, mouths and face details, a flower garland, medal, pearls and tie, a kite, rabana drum, balloons, easel and jetpack, and **10 fabric prints** (stripes, polka dots, gingham, flowers, batik, zigzag, stars, camouflage, tartan, waves) for tops and bottoms. **Garage: 99 items** (from 55): 6 decals, 3 spoilers, 5 underglows, 4 wheels, 2 exhausts, 6 roof loads, 3 engines, 4 horns, and the 10 prints as **car wraps**. Prints follow the model, so they don't slide as you move. There were already 30 named mission givers along the chapters (plus the city mission-chain characters), so the 25-character target was met before this | ✅ | `src/world/Pockets.ts`, `src/gameplay/Trophies.ts`, `src/audio/AudioEngine.ts`, `src/render/PaintMaterial.ts`, `src/models/Human.ts`, `src/models/Vehicles.ts` |
| **Season pass, sponsor challenges and a cosmetic-only shop**: Menu → *Season pass*. Seasons last 8 weeks and have 30 tiers. Season points come from ordinary play (laps, missions, pockets, stunts, photos, races, trophies…), counted from when the season starts. The **free track** gives ink on most tiers and a cosmetic every fifth tier. The **Patron track** gives a cosmetic on every tier and never gives ink. It is unlocked with one-use codes or an admin grant (admin → *Pass & challenges*). There is **no payment provider**: `server/store.mjs` has the single `grant()` hook a payment webhook would call. **Sponsor challenges** are set up by the admin per sponsor ("travel 10 km", "land 20 stunts"…, with ink and an optional cosmetic). Players join them and progress counts from joining; the admin sees how many joined and finished. **Shop**: six featured cosmetics a day at 25 % off. Nothing a pass, code or sponsor gives can be a vehicle or a tonic, and tests enforce this | ✅ | `src/gameplay/SeasonPass.ts`, `src/ui/PassScreen.ts`, `server/store.mjs`, `server/admin.mjs` |
...
**Tested here:** 235 unit tests (141 at the start of this milestone), including building and dressing a Road Studio road in all 28 styles. Also checked in the browser:
