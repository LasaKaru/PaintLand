...
describe('Localisation (24 languages)', () => {
  ...
  it('non-English languages are actually translated (not copies of English)', () => {
    ...
    for (const l of LANGS) {
      ...
      const international = new Set(['item.top:tee', 'item.top:sari', 'item.bottom:sarong', 'item.facial:bindi', 'item.mouth:o', 'item.hair:bob', 'item.roof:kayak', 'emote.ayubowan', 'wr.presetOsariya', 'acct.club', 'acct.password', 'acct.title']);
